...
if ($totalRows_networks > 0) { // Show if recordset not empty ?>
              ...
              <?php
				...
				?>
        	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="document.getElementById('parent').value = '<?php echo $row_networks['id']; ?>'; document.getElementById('searchQ_1').style.display = 'none'; document.getElementById('selected_network').style.display = 'block'; document.getElementById('selected_network').value = '<?php if ($row_networks['v6mask'] == "") { ?><?php echo long2ip($row_networks['network']); ?><?php if ($row_networks['mask'] == "255.255.255.255") { echo "/32"; } else { echo get_slash($row_networks['mask']); } ?><?php } else { ?><?php echo Net_IPv6::compress(long2ipv6($row_networks['network'])); ?><?php echo "/".$row_networks['v6mask']; ?><?php } ?>'; document.getElementById('network_search').value = '<?php echo $row_networks['descr']; ?>';"><?php if ($row_networks['v6mask'] == "") { ?><strong><?php echo long2ip($row_networks['network']); ?><?php if ($row_networks['mask'] == "255.255.255.255") { echo "/32"; } else { echo get_slash($row_networks['mask']); } ?></strong><?php } else { ?><strong><?php echo Net_IPv6::compress(long2ipv6($row_networks['network'])); ?><?php echo "/".$row_networks['v6mask']; ?></strong><?php } ?></a> <?php echo $row_networks['descr']; ?><br>
              <?php } ?>
